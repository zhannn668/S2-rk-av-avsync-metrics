...
// Nearest-rank percentile. q in [0,1]. return RK_NAN if n==0
...
void avsync_on_video(AvSync *s, uint64_t video_pts_us)
{
    ...
    if (!s->has_video0) {
        ...
        s->video0_us = video_pts_us;
         // ---- compute paired offset/residual on VIDEO events (use latest audio as master clock) ----
    if (s->has_last_audio) {
...
void avsync_on_audio(AvSync *s, uint64_t audio_pts_us, uint32_t frames, uint32_t sample_rate)
{
    ...
    if (s->has_last_audio && audio_pts_us > s->last_audio_us && s->has_last_audio_meta) {
        uint64_t delta_us = audio_pts_us - s->last_audio_us;

        // expected delta = previous chunk frames / previous chunk sample_rate
